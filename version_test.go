...
func TestNormalizedVersion(t *testing.T) {
	...
	for _, version := range normalizedVersions {
		t.Run(version.original, func(t *testing.T) {
			...
			if v.Complete() != version.normalized {
				t.Errorf("original: %s, %s(actual) != %s(expected)", version.original, v.Complete(), version.normalized)
			}
		})
	}
}
...
func TestVersionString(t *testing.T) {
	...
	for _, str := range versionStrings {
		t.Run(str.version, func(t *testing.T) {
			var v IVersion
	...
	for _, public := range versionPublics {
		t.Run(public.version, func(t *testing.T) {
			...
			if v.Public() != public.public {
				t.Errorf("version: %s, %s(actual) != %s(expected)", public.version, v.Public(), public.public)
			}
		})
	}
}
...
func TestVersionBase(t *testing.T) {
	...
	for _, base := range versionBases {
		t.Run(base.version, func(t *testing.T) {
			...
			if v.Base() != base.base {
				t.Errorf("version: %s, %s(actual) != %s(expected)", base.version, v.Base(), base.base)
			}
		})
	}
}
...
func TestVersionEpoch(t *testing.T) {
	...
	for _, epoch := range versionEpochs {
		t.Run(epoch.version, func(t *testing.T) {
			...
			if v.Epoch() != epoch.epoch {
				t.Errorf("version: %s, %d(actual) != %d(expected)", epoch.version, v.Epoch(), epoch.epoch)
			}
		})
	}
}
	...
	for _, release := range versionReleases {
		t.Run(release.version, func(t *testing.T) {
			...
			if !reflect.DeepEqual(v.Release(), release.release) {
				t.Errorf("version: %s, %v(actual) != %v(expected)", release.version, v.Release(), release.release)
			}
		})
	}
}
...
func TestVersionPre(t *testing.T) {
	...
	for _, pre := range versionPres {
		t.Run(pre.version, func(t *testing.T) {
			...
			if !reflect.DeepEqual(v.Pre(), pre.pre) {
				t.Errorf("version: %s, %v(actual) != %v(expected)", pre.version, v.Pre(), pre.pre)
			}
		})
	}
}
...
func TestVersionPost(t *testing.T) {
	...
	for _, post := range versionPosts {
		t.Run(post.version, func(t *testing.T) {
			...
			if !reflect.DeepEqual(v.Post(), post.post) {
				t.Errorf("version: %s, %v(acutal) != %v(expected)", post.version, v.Post(), post.post)
			}
		})
	}
}
	...
	for _, dev := range versionDevs {
		t.Run(dev.version, func(t *testing.T) {
			...
			if !reflect.DeepEqual(v.Dev(), dev.dev) {
				t.Errorf("version: %s, %v(expected) != %v(actual)", dev.version, v.Dev(), dev.dev)
			}
		})
	}
}
